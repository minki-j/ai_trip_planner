...
export default function ScheduleDisplay({ activities }: ScheduleDisplayProps) {
  // Calculate earliest and latest dates from activities
  const dateRange = activities.reduce(
    (acc, activity) => {
    ...
    },
    {
      earliest: new Date(
        Math.max(
          ...activities.map((a) => new Date(a.time.start_time).getTime())
        )
      ),
      latest: new Date(
        Math.min(
  ...
  const currentActivities = activities.filter((activity) =>
    isSameDay(new Date(activity.time.start_time), currentDate)
  );
